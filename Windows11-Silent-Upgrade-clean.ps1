# Windows 11 Hardware Bypass & Installation Script v4.0# Windows 11 Hardware Bypass & Auto-Upgrade Script v3.4

# Specifically designed to handle error 0xa0000400 (hardware compatibility)# Automated Windows 10 to 11 upgrade with Installation Assistant bypass + PC Health Check automation

# Comprehensive TPM, Secure Boot, and CPU compatibility bypass# Automatically handles PC Health Check app requirement + bypasses Installation Assistant hardware checks

# Enhanced executable location detection with comprehensive search methods

param(# Shows all operations and progress in PowerShell and Installation Assistant

    [switch]$Force,# Based on Ventoy's Windows11Bypass implementation with comprehensive bypass integration

    [switch]$NoReboot,

    [string]$LogPath = "$env:TEMP\Windows11-Upgrade.log"# Ensure script execution is allowed

)Set-ExecutionPolicy -Scope Process -ExecutionPolicy Bypass -Force



# Global configuration# Global variables for enhanced functionality

$script:LogFile = $LogPath$global:LogFile = "$env:TEMP\Windows11-Upgrade-Log.txt"

$script:DownloadPath = "$env:TEMP\Windows11Assistant"$global:MaxRetries = 3

$script:AssistantURL = "https://go.microsoft.com/fwlink/?linkid=2171764"$global:DownloadTimeout = 1800  # 30 minutes

$script:MaxRetries = 3$global:IsAutomatedMode = $true



# Enhanced logging function# Enhanced logging function

function Write-LogMessage {function Write-LogMessage {
    ...
    $logEntry = "[$timestamp] [$Level] $Message"    $logEntry = "[$timestamp] [$Level] $Message"

        

    Write-Host $Message -ForegroundColor $Color    # Write to console with color

        Write-Host $Message -ForegroundColor $Color

    try {    

        Add-Content -Path $script:LogFile -Value $logEntry -Encoding UTF8 -ErrorAction SilentlyContinue    # Write to log file

    } catch {    try {

        # Silent continue on log errors        Add-Content -Path $global:LogFile -Value $logEntry -ErrorAction SilentlyContinue
...
function Test-Administrator {
    ...
    $currentUser = [Security.Principal.WindowsIdentity]::GetCurrent()# System validation function

    $principal = New-Object Security.Principal.WindowsPrincipal($currentUser)function Test-SystemCompatibility {

    return $principal.IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)    Write-LogMessage "Performing system compatibility check..."[OK] "INFO"[OK] "Cyan"

}    

    $issues = @()

# Comprehensive registry bypass for error 0xa0000400    

function Set-Windows11RegistryBypass {    # Check if running as Administrator

    Write-LogMessage "Applying Windows 11 hardware bypass registry entries..." "INFO" "Yellow"    $isAdmin = ([Security.Principal.WindowsPrincipal] [Security.Principal.WindowsIdentity]::GetCurrent()).IsInRole([Security.Principal.WindowsBuiltInRole] "Administrator")

        if (-not $isAdmin) {

    try {        $issues += "Script must be run as Administrator"

        # Primary bypass registry keys for Installation Assistant    }

        $registryPaths = @{    

            "HKLM:\SYSTEM\Setup\MoSetup" = @{    # Check Windows version

                "AllowUpgradesWithUnsupportedTPMOrCPU" = 1    $osVersion = [System.Environment]::OSVersion.Version

            }    $buildNumber = (Get-ItemProperty "HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion").CurrentBuild

            "HKLM:\SYSTEM\Setup\LabConfig" = @{    

                "BypassTPMCheck" = 1    if ($osVersion.Major -lt 10) {

                "BypassSecureBootCheck" = 1        $issues += "Windows 10 or higher required"

                "BypassRAMCheck" = 1    }

                "BypassStorageCheck" = 1    

                "BypassCPUCheck" = 1    # Check if already Windows 11

            }    if ($buildNumber -ge 22000) {

            "HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion" = @{        Write-LogMessage "System is already running Windows 11 (Build: $buildNumber)"[OK] "WARNING"[OK] "Yellow"

                "EditionID" = "Professional"        return $true

                "ProductName" = "Windows 10 Pro"    }

            }    

            "HKLM:\SYSTEM\CurrentControlSet\Control\SecureBoot\State" = @{    # Check available disk space (minimum 20GB)

                "UEFISecureBootEnabled" = 1    $systemDrive = $env:SystemDrive
        ...
        foreach ($regPath in $registryPaths.Keys) {    if ($freeSpaceGB -lt 20) {

            try {        $issues += "Insufficient disk space. Available: $freeSpaceGB GB, Required: 20 GB"

                if (!(Test-Path $regPath)) {    }

                    New-Item -Path $regPath -Force | Out-Null    

                    Write-LogMessage "Created registry path: $regPath" "SUCCESS" "Green"    # Check internet connectivity

                }    try {

                        $null = Test-NetConnection -ComputerName "go.microsoft.com"[OK] -Port 443 -InformationLevel Quiet

                $values = $registryPaths[$regPath]    } catch {

                foreach ($valueName in $values.Keys) {        $issues += "No internet connectivity detected"

                    $value = $values[$valueName]    }

                    Set-ItemProperty -Path $regPath -Name $valueName -Value $value -Type DWord -Force    

                    Write-LogMessage "Set $regPath\$valueName = $value" "SUCCESS" "Gray"    if ($issues.Count -gt 0) {

                }        Write-LogMessage "System compatibility issues found:"[OK] "ERROR"[OK] "Red"

            } catch {        foreach ($issue in $issues) {

                Write-LogMessage "Failed to modify $regPath : $($_.Exception.Message)" "WARNING" "Yellow"            Write-LogMessage "[OK]  • $issue"[OK] "ERROR"[OK] "Red"
        ...
        # Additional TPM and hardware simulation    

        $tpmPaths = @{    Write-LogMessage "✓ System compatibility check passed"[OK] "SUCCESS"[OK] "Green"

            "HKLM:\SYSTEM\CurrentControlSet\Services\TPM\WMI" = @{    Write-LogMessage "Current Windows version: $($osVersion.Major).$($osVersion.Minor) Build $buildNumber"[OK] "INFO"[OK] "Gray"

                "TpmPresent" = 1    Write-LogMessage "Available disk space: $freeSpaceGB GB"[OK] "INFO"[OK] "Gray"

                "TpmReady" = 1    return $true

                "TpmEnabled" = 1}

                "TpmActivated" = 1

                "TpmVersion" = "2.0"# Enhanced download function with progress and retry

            }function Download-FileWithProgress {

            "HKLM:\SYSTEM\CurrentControlSet\Control\DeviceGuard" = @{    param(

                "EnableVirtualizationBasedSecurity" = 0        [string]$Url,

                "RequirePlatformSecurityFeatures" = 0        [string]$OutFile,
        ...
        foreach ($regPath in $tpmPaths.Keys) {    

            try {    for ($attempt = 1; $attempt -le $MaxRetries; $attempt++) {

                if (!(Test-Path $regPath)) {        try {

                    New-Item -Path $regPath -Force | Out-Null            Write-LogMessage "Download attempt $attempt of $MaxRetries..."[OK] "INFO"[OK] "Yellow"

                }            Write-LogMessage "URL: $Url"[OK] "INFO"[OK] "Gray"

                            Write-LogMessage "Destination: $OutFile"[OK] "INFO"[OK] "Gray"

                $values = $tpmPaths[$regPath]            

                foreach ($valueName in $values.Keys) {            # Use BITS transfer for better reliability with visible progress

                    $value = $values[$valueName]            try {

                    $valueType = if ($value -is [string]) { "String" } else { "DWord" }                Import-Module BitsTransfer -ErrorAction Stop

                    Set-ItemProperty -Path $regPath -Name $valueName -Value $value -Type $valueType -Force                Write-LogMessage "Starting Windows 11 Installation Assistant download with progress..."[OK] "INFO"[OK] "Yellow"

                    Write-LogMessage "Set TPM bypass: $regPath\$valueName = $value" "SUCCESS" "Gray"                

                }                # Start BITS transfer with visible progress monitoring

            } catch {                $job = Start-BitsTransfer -Source $Url -Destination $OutFile -Description "Windows 11 Installation Assistant"[OK] -DisplayName "Windows 11 Download"[OK] -Asynchronous

                Write-LogMessage "TPM registry modification failed: $($_.Exception.Message)" "WARNING" "Yellow"                
        ...
        Write-LogMessage "Registry bypass configuration completed successfully" "SUCCESS" "Green"                    $job = Get-BitsTransfer -JobId $job.JobId

        return $true                    if ($job.BytesTotal -gt 0) {

                                $percentComplete = [math]::Round(($job.BytesTransferred / $job.BytesTotal) * 100, 1)

    } catch {                        Write-LogMessage "Download progress: $percentComplete% ($([math]::Round($job.BytesTransferred / 1MB, 1)) MB / $([math]::Round($job.BytesTotal / 1MB, 1)) MB)"[OK] "INFO"[OK] "Cyan"

        Write-LogMessage "Registry bypass failed: $($_.Exception.Message)" "ERROR" "Red"                    }

        return $false                } while ($job.JobState -eq "Transferring")
...
# Download Windows 11 Installation Assistant                    Write-LogMessage "✓ Download completed using BITS transfer"[OK] "SUCCESS"[OK] "Green"

function Get-Windows11Assistant {                    return $true

    Write-LogMessage "Downloading Windows 11 Installation Assistant..." "INFO" "Yellow"                } else {

                        Remove-BitsTransfer -BitsJob $job

    try {                    throw "BITS transfer failed with state: $($job.JobState)"

        # Create download directory                }

        if (!(Test-Path $script:DownloadPath)) {            } catch {

            New-Item -ItemType Directory -Path $script:DownloadPath -Force | Out-Null                Write-LogMessage "BITS transfer failed, falling back to WebRequest with progress..."[OK] "WARNING"[OK] "Yellow"

        }                

                        # Fallback to WebRequest with visible progress

        $assistantPath = Join-Path $script:DownloadPath "Windows11InstallationAssistant.exe"                try {

                            $webClient = New-Object System.Net.WebClient

        # Remove existing file if present                    $webClient.add_DownloadProgressChanged({

        if (Test-Path $assistantPath) {                        param($sender, $e)

            Remove-Item $assistantPath -Force -ErrorAction SilentlyContinue                        Write-LogMessage "Download progress: $($e.ProgressPercentage)% ($([math]::Round($e.BytesReceived / 1MB, 1)) MB / $([math]::Round($e.TotalBytesToReceive / 1MB, 1)) MB)"[OK] "INFO"[OK] "Cyan"

        }                    })

                            

        # Download with progress                    Write-LogMessage "Starting download with progress monitoring..."[OK] "INFO"[OK] "Yellow"

        $webClient = New-Object System.Net.WebClient                    $webClient.DownloadFileAsync($Url, $OutFile)

        $webClient.DownloadFile($script:AssistantURL, $assistantPath)                    

        $webClient.Dispose()                    # Wait for download to complete

                            do {

        if (Test-Path $assistantPath) {                        Start-Sleep -Seconds 1

            $fileSize = (Get-Item $assistantPath).Length                    } while ($webClient.IsBusy)

            Write-LogMessage "Download completed. File size: $([math]::Round($fileSize/1MB, 2)) MB" "SUCCESS" "Green"                    

            return $assistantPath                    $webClient.Dispose()

        } else {                    Write-LogMessage "✓ Download completed using WebRequest"[OK] "SUCCESS"[OK] "Green"

            throw "Download failed - file not found"                    return $true

        }                } catch {

                            Write-LogMessage "WebRequest download also failed: $($_.Exception.Message)"[OK] "ERROR"[OK] "Red"

    } catch {                    throw "All download methods failed"
...
# Start Windows 11 upgrade with bypasses            if ($attempt -lt $MaxRetries) {

function Start-Windows11Upgrade {                $waitTime = $attempt * 30

    param([string]$AssistantPath)                Write-LogMessage "Waiting $waitTime seconds before retry..."[OK] "INFO"[OK] "Yellow"

                    Start-Sleep -Seconds $waitTime

    Write-LogMessage "Starting Windows 11 upgrade process..." "INFO" "Cyan"            }

            }

    try {    }

        # Apply registry bypasses before starting    

        if (!(Set-Windows11RegistryBypass)) {    Write-LogMessage "All download attempts failed"[OK] "ERROR"[OK] "Red"

            Write-LogMessage "Registry bypass failed - continuing anyway" "WARNING" "Yellow"    return $false

        }}

        

        # Start Installation Assistant with bypass parameters# Installation Assistant specific bypass function

        $processArgs = @(function Set-InstallationAssistantBypass {

            "/quietinstall",    Write-LogMessage "Setting enhanced Installation Assistant bypass entries for error 0xa0000400..."[OK] "INFO"[OK] "Cyan"

            "/skipeula",    

            "/auto",     try {

            "upgrade",        # Comprehensive bypass for Installation Assistant error 0xa0000400

            "/copylogs", $script:DownloadPath        # This error specifically indicates hardware compatibility detection failure

        )        

                # Step 1: Core hardware bypass registry entries

        Write-LogMessage "Launching Windows 11 Installation Assistant with hardware bypasses..." "INFO" "Yellow"        $assistantPaths = @{

        Write-LogMessage "Command: $AssistantPath $($processArgs -join ' ')" "INFO" "Gray"            "HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion"[OK] = @{

                        "CurrentBuild"[OK] = 19045  # Report as recent Windows 10 build

        $process = Start-Process -FilePath $AssistantPath -ArgumentList $processArgs -PassThru -NoNewWindow                "CurrentBuildNumber"[OK] = "19045"

                        "CurrentVersion"[OK] = "10.0"

        if ($process) {                "ProductName"[OK] = "Windows 10 Pro"

            Write-LogMessage "Installation Assistant started (PID: $($process.Id))" "SUCCESS" "Green"            }

            Write-LogMessage "Monitoring upgrade progress..." "INFO" "Cyan"            "HKLM:\SYSTEM\CurrentControlSet\Control\Firmware\Security"[OK] = @{

                            "SecureBootEnabled"[OK] = 1

            # Monitor process                "SecureBootCapable"[OK] = 1

            while (!$process.HasExited) {            }

                Start-Sleep -Seconds 30            "HKLM:\SYSTEM\CurrentControlSet\Services\TPM\WMI"[OK] = @{

                Write-LogMessage "Upgrade in progress... (Process running)" "INFO" "Cyan"                "TpmPresent"[OK] = 1

            }                "TpmReady"[OK] = 1

                            "TpmEnabled"[OK] = 1

            $exitCode = $process.ExitCode                "TpmActivated"[OK] = 1

            Write-LogMessage "Installation Assistant completed with exit code: $exitCode" "INFO" "White"                "TpmVersion"[OK] = "2.0"

                        }

            if ($exitCode -eq 0) {            "HKLM:\SYSTEM\CurrentControlSet\Control\DeviceGuard"[OK] = @{

                Write-LogMessage "Windows 11 upgrade completed successfully!" "SUCCESS" "Green"                "EnableVirtualizationBasedSecurity"[OK] = 0

                return $true                "RequirePlatformSecurityFeatures"[OK] = 0

            } else {            }

                Write-LogMessage "Upgrade failed with exit code: $exitCode" "ERROR" "Red"        }

                return $false        

            }        # Step 2: Additional registry paths for error 0xa0000400

        } else {        $errorSpecificPaths = @{

            throw "Failed to start Installation Assistant process"            "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Setup\OOBE"[OK] = @{
        ...
        Write-LogMessage "Upgrade process failed: $($_.Exception.Message)" "ERROR" "Red"            "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Setup\State"[OK] = @{

        return $false                "ImageState"[OK] = "IMAGE_STATE_COMPLETE"
...
# Additional registry cleanup for stubborn compatibility checks                "GeneralizationState"[OK] = 7

function Clear-CompatibilityCache {                "CleanupState"[OK] = 2

    Write-LogMessage "Clearing Windows compatibility cache..." "INFO" "Yellow"            }

                "HKLM:\SYSTEM\CurrentControlSet\Control\Session Manager\Memory Management"[OK] = @{

    try {                "FeatureSettings"[OK] = 1

        $cachePaths = @(                "FeatureSettingsOverride"[OK] = 3

            "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\AppCompatFlags\Compatibility Assistant\Store",                "FeatureSettingsOverrideMask"[OK] = 3

            "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\AppCompatFlags\TelemetryController",            }

            "HKCU:\SOFTWARE\Microsoft\Windows\CurrentVersion\AppCompatFlags\Compatibility Assistant\Store"            "HKLM:\SYSTEM\CurrentControlSet\Control\CI\Config"[OK] = @{
        ...
        foreach ($cachePath in $cachePaths) {            }
            ...
            try {        }
                ...
                if (Test-Path $cachePath) {        
                    ...
                    Remove-Item -Path $cachePath -Recurse -Force -ErrorAction SilentlyContinue        # Combine all registry paths

                    Write-LogMessage "Cleared cache: $cachePath" "SUCCESS" "Gray"        $allPaths = $assistantPaths + $errorSpecificPaths

                }        

            } catch {        # Create and set Installation Assistant bypass registry entries

                Write-LogMessage "Could not clear cache: $cachePath" "WARNING" "Yellow"        foreach ($regPath in $allPaths.Keys) {
        ...
        # Force refresh Windows compatibility database                    New-Item -Path $regPath -Force -ErrorAction Stop | Out-Null

        try {                    Write-LogMessage "Created registry path: $regPath"[OK] "SUCCESS"[OK] "Gray"

            Start-Process -FilePath "sfc" -ArgumentList "/scannow" -WindowStyle Hidden -Wait -ErrorAction SilentlyContinue                }

        } catch {                

            # Silent continue                # Set all values for this path

        }                $values = $allPaths[$regPath]

                        foreach ($valueName in $values.Keys) {

        return $true                    $value = $values[$valueName]

                            try {

    } catch {                        if ($value -is [string]) {

        Write-LogMessage "Cache clearing failed: $($_.Exception.Message)" "WARNING" "Yellow"                            Set-ItemProperty -Path $regPath -Name $valueName -Value $value -Type String -Force -ErrorAction Stop
...
# Main execution function                    } catch {

function Start-Windows11UpgradeProcess {                        Write-LogMessage "Could not set $regPath\$valueName : $($_.Exception.Message)"[OK] "WARNING"[OK] "Yellow"

    Write-LogMessage "Windows 11 Upgrade Script v4.0 - Hardware Bypass Edition" "INFO" "Cyan"                    }

    Write-LogMessage "Designed to handle error 0xa0000400 and hardware compatibility issues" "INFO" "White"                }

    Write-LogMessage "Starting upgrade process..." "INFO" "Yellow"            } catch {
    ...
    if (!(Test-Administrator)) {        }
        ...
        Write-LogMessage "ERROR: This script must be run as Administrator" "ERROR" "Red"        

        Write-LogMessage "Please right-click and select 'Run as Administrator'" "ERROR" "Red"        # Step 3: Installation Assistant compatibility flags and error-specific overrides
    ...
    # Check Windows version                New-Item -Path $compFlags -Force -ErrorAction Stop | Out-Null

    $osVersion = [System.Environment]::OSVersion.Version            }

    if ($osVersion.Major -lt 10) {            

        Write-LogMessage "ERROR: Windows 10 or later is required" "ERROR" "Red"            # Enhanced Installation Assistant compatibility overrides for error 0xa0000400

        return $false            $iaCompatValues = @{

    }                "Windows11InstallationAssistant.exe"[OK] = "~ RUNASADMIN WIN11COMPAT DISABLETHEMES"

                    "Windows11Upgrade"[OK] = "COMPATIBLE"

    Write-LogMessage "Administrator check passed" "SUCCESS" "Green"                "HardwareCompatibilityOverride"[OK] = 1

    Write-LogMessage "Windows version check passed" "SUCCESS" "Green"                "SkipCompatibilityCheck"[OK] = 1

                    "BypassHardwareCheck"[OK] = 1

    # Clear compatibility cache first            }

    Clear-CompatibilityCache            

                foreach ($flag in $iaCompatValues.GetEnumerator()) {

    # Apply registry bypasses                try {

    Write-LogMessage "Applying hardware compatibility bypasses..." "INFO" "Yellow"                    Set-ItemProperty -Path $compFlags -Name $flag.Key -Value $flag.Value -Force -ErrorAction Stop

    if (!(Set-Windows11RegistryBypass)) {                    Write-LogMessage "Set compatibility flag: $($flag.Key)"[OK] "SUCCESS"[OK] "Gray"

        Write-LogMessage "WARNING: Some registry bypasses failed" "WARNING" "Yellow"                } catch {

        if (!$Force) {                    Write-LogMessage "Could not set compatibility flag $($flag.Key): $($_.Exception.Message)"[OK] "WARNING"[OK] "Yellow"

            Write-LogMessage "Use -Force parameter to continue anyway" "WARNING" "Yellow"                }
    ...
    $assistantPath = Get-Windows11Assistant        # Step 4: Force clear all compatibility caches that might trigger error 0xa0000400

    if (!$assistantPath) {        try {

        Write-LogMessage "ERROR: Failed to download Windows 11 Installation Assistant" "ERROR" "Red"            $cacheKeys = @(

        return $false                "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Setup\State",
    ...
    # Start upgrade process                "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\AppCompatFlags\Layers",

    Write-LogMessage "All prerequisites met. Starting Windows 11 upgrade..." "SUCCESS" "Green"                "HKCU:\SOFTWARE\Microsoft\Windows\CurrentVersion\AppCompatFlags\Layers"

    $upgradeResult = Start-Windows11Upgrade -AssistantPath $assistantPath            )

                

    if ($upgradeResult) {            foreach ($cacheKey in $cacheKeys) {

        Write-LogMessage "Windows 11 upgrade process completed successfully!" "SUCCESS" "Green"                try {

        Write-LogMessage "Your system should now be upgraded to Windows 11" "SUCCESS" "Green"                    if (Test-Path $cacheKey) {

                                Remove-Item -Path $cacheKey -Recurse -Force -ErrorAction Stop

        if (!$NoReboot) {                        Write-LogMessage "Cleared compatibility cache: $cacheKey"[OK] "SUCCESS"[OK] "Gray"

            Write-LogMessage "System will reboot in 60 seconds..." "INFO" "Yellow"                    }

            Start-Sleep -Seconds 10                } catch {

            Write-LogMessage "Rebooting system..." "INFO" "Yellow"                    Write-LogMessage "Could not clear cache $cacheKey : $($_.Exception.Message)"[OK] "WARNING"[OK] "Yellow"
        ...
        Write-LogMessage "Windows 11 upgrade failed. Check the logs for details." "ERROR" "Red"            Write-LogMessage "Could not clear compatibility caches: $($_.Exception.Message)"[OK] "WARNING"[OK] "Yellow"

        Write-LogMessage "Log file: $script:LogFile" "INFO" "White"        }

    }        

            # Step 5: Additional CPU and platform compatibility overrides for 0xa0000400

    return $upgradeResult        try {

}            $cpuCompatPath = "HKLM:\HARDWARE\DESCRIPTION\System\CentralProcessor\0"

            if (Test-Path $cpuCompatPath) {

# Script entry point                # Override CPU identification to supported model

if ($MyInvocation.InvocationName -ne '.') {                Set-ItemProperty -Path $cpuCompatPath -Name "ProcessorNameString"[OK] -Value "Intel(R) Core(TM) i7-8700K CPU at 3.70GHz"[OK] -Type String -Force -ErrorAction SilentlyContinue

    # Only run if script is executed directly (not dot-sourced)                Set-ItemProperty -Path $cpuCompatPath -Name "Identifier"[OK] -Value "Intel64 Family 6 Model 158 Stepping 10"[OK] -Type String -Force -ErrorAction SilentlyContinue

    try {                Write-LogMessage "Set CPU compatibility override for 0xa0000400"[OK] "SUCCESS"[OK] "Gray"

        $result = Start-Windows11UpgradeProcess            }

        if ($result) {        } catch {

            Write-Host "SUCCESS: Windows 11 upgrade completed!" -ForegroundColor Green            Write-LogMessage "Could not set CPU compatibility override: $($_.Exception.Message)"[OK] "WARNING"[OK] "Yellow"

            exit 0        

        } else {        Write-LogMessage "✓ Enhanced Installation Assistant bypass for error 0xa0000400 completed"[OK] "SUCCESS"[OK] "Green"

            Write-Host "FAILED: Windows 11 upgrade failed!" -ForegroundColor Red        return $true

            exit 1        

        }    } catch {

    } catch {        Write-LogMessage "Enhanced Installation Assistant bypass failed: $($_.Exception.Message)"[OK] "ERROR"[OK] "Red"

        Write-Host "CRITICAL ERROR: $($_.Exception.Message)" -ForegroundColor Red        return $false
